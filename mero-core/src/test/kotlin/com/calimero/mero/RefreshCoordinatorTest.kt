...
class RefreshCoordinatorTest {
    ...
    private fun store(access: String, refresh: String) =
        MemoryTokenStore().apply { setTokens(TokenData(access, refresh, Long.MAX_VALUE)) }

    @Test
    fun `concurrent refreshes collapse into a single network call`() = runBlocking {
    ...
    @Test
    fun `adopts a bundle another process already rotated instead of replaying`() = runBlocking {
