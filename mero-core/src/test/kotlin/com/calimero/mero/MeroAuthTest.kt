...
class MeroAuthTest {
    ...
    @Test
    fun `authenticate posts the mero request shape and stores tokens`() =
        runBlocking {
            server.enqueue(
    ...
    @Test
    fun `401 token_expired triggers a single refresh then retries the request`() =
        runBlocking {
    ...
    @Test
    fun `terminal x-auth-error clears the store and throws AuthRevokedException`() =
        runBlocking {
            mero.setTokenData(TokenData("old.access", "refresh1", Long.MAX_VALUE))
