...
<Aside type="caution">
  `updateKeyPermissions` applies a **delta**, not a replacement: core removes
  `remove` first, then adds `add`. A `permissions` field is ignored, so sending
  the full desired set silently does nothing.

  ```kotlin
  mero.auth.updateKeyPermissions(keyId, UpdateKeyPermissionsRequest(add = listOf("admin")))
  ```
</Aside>
