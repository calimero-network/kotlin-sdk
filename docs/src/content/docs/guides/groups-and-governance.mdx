...
// Who this node is. `getNamespaceIdentity` is gone: core#3522 deleted the route
// that asked a namespace — "ask the node who it is" — and it 404s.
val identity = mero.admin.getNodeIdentity()
```

`upgradePolicy` is not a parameter any more: core#3485 removed the concept, and
it always held `"LazyOnAccess"`.
...
Joining is only half the story: the group's **contexts** are not initialized on
...
[`syncGroupContexts`](/reference/admin-api/#syncgroupcontexts--the-anti-uninitialized-context-helper),
...
If it stays empty, check `getPeersCount()` — 0 peers means this node has no one
